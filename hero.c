...
/* Start addresses of the vertical rows spanning 6 pixels: 33 rows (0..32), containing 40 cells (00..39) spanning 6 pixels */
...
#define is_shifted() ( ((unsigned char) *0x20C) & 128)	// #FF = Majuscule pressée, #7F = minuscule pressée
	...
	draw_hero_hovering_right(x, y, 1);  // DRAW SPRITE at initial position
	 ...
	 for(; key != KEY_SPACE;) {
		...
		if(key == KEY_RIGHT) {
			...
			else { // We were previously going LEFT  => Just REVERSE current direction and redraw new sprite
				xdir = XDIR_RIGHT;
			...
			else { // We were previously going RIGHT => Just REVERSE current direction and redraw new sprite
				xdir = XDIR_LEFT;
